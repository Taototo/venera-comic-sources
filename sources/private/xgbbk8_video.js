...
class PrivateXgbbk8Video extends ComicSource {
  type = "video";
  name = "XGBBK8资源（私人）";
  key = "private_xgbbk8_video";
  version = "1.0.0";
  minAppVersion = "1.0.0";
  url = "https://cdn.jsdelivr.net/gh/Taototo/venera-comic-sources@main/sources/private/xgbbk8_video.js";

  settings = {
    api: {
      title: "JSON 接口",
      type: "input",
      default: "https://json.xgbbk8.com/api.php/provide/vod/",
    },
  };
  ...
  async request(params) {
    ...
    if (res.status !== 200) throw `XGBBK8接口状态异常: ${res.status}`;
    let text = String(res.body || "").trim();
    if (!text || text[0] !== "{") throw "XGBBK8返回了无法解析的数据";
    text = text.replace(/("vod_id"\s*:\s*)(\d+)/g, '$1"$2"');
    let data;
    try {
      data = JSON.parse(text);
    } catch (_) {
      throw "XGBBK8返回了无效 JSON";
    }
    if (data.code !== undefined && Number(data.code) !== 1) {
      throw data.msg || "XGBBK8接口返回错误";
    }
    return data;
  }
  ...
  category = {
    ...
    parts: [
      ...
      this.categoryPart("视频四区", [["全部", 79], ["吃瓜黑料", 80], ["酒店探花", 81], ["直播裸聊", 82], ["反差母狗", 83], ["颜值正义", 84], ["熟女少妇", 85], ["人兽乱交", 86], ["国产传媒", 89]]),
      this.categoryPart("视频五区", [["全部", 90], ["野战车震", 91], ["SM调教", 92], ["家庭乱伦", 93], ["百合女同", 94], ["学生空姐", 95], ["撸管必看", 96], ["偷情少妇", 97]]),
  ...
  comic = {
    ...
    loadEp: async (comicId, epId) => {
      ...
      if (!videoUrl) throw "XGBBK8当前集数没有可用的视频地址";
      return {
        images: [
          `venera-video:${JSON.stringify({ url: videoUrl, title: "XGBBK8资源", headers: this.headers })}`,
        ],
      };
    },
  };
